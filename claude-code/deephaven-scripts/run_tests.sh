...
#
# Invoked by `./gradlew :deephaven-scripts:pytest` (see build.gradle.kts) and
...
if ! command -v "$PYTHON_BIN" >/dev/null 2>&1; then
  ...
  echo "       The :deephaven-scripts module needs Python 3.10+ to run its unit tests." >&2
...
if ! "$PYTHON_BIN" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 10) else 1)'; then
  ...
  echo "       fix42cache requires Python 3.10+ (PEP 604 unions). Set PYTHON=/path/to/python3.10+." >&2
  exit 1
fi

VENV_DIR="$MODULE_DIR/.venv"
if [ ! -d "$VENV_DIR" ]; then
  echo "[deephaven-scripts] creating virtualenv at $VENV_DIR"
