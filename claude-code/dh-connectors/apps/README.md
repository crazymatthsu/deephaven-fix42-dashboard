...
application that needs code of its own — a `RecordTransform` bean for its
`transforms:` list (doc 07 §12), a bespoke decoder, an extra health indicator.
...
a `build.gradle.kts`), `./gradlew :dh-connectors:apps:<app-name>:dockerBuildLocal`
