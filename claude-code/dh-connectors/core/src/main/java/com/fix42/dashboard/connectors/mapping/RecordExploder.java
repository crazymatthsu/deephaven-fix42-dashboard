...
public final class RecordExploder {
     ...
     */
    public List<MappedRow> explode(SourceRecord record, Map<String, String> fields, Instant ingestTime) {
    ...
    private List<MappedRow> deletes(SourceRecord record, Map<String, String> fields, Instant ingestTime) {
    ...
    private List<String> replaceTracked(
            SourceRecord record, Map<String, String> fields, Instant ingestTime, List<String> members) {
        if (!schema.keyed()) {
            return List.of();
        }
        String identity = recordIdentity(record, fields, ingestTime);
     ...
     */
    private String recordIdentity(SourceRecord record, Map<String, String> fields, Instant ingestTime) {
        if (record.key() != null) {
            return "sow:" + record.key();
        }
        if (recordKeyIndexes.length == 0) {
            return null;
        }
        Object[] values = mapper.map(record.withAction(SourceRecord.Action.UPSERT), fields, ingestTime)
                .values();
    ...
    private MappedRow deleteRow(
            SourceRecord record, Map<String, String> fields, String member, Instant ingestTime) {
        Map<String, String> augmented = new LinkedHashMap<>(fields);
        augmented.put(KEY_TAG, member);
        return mapper.map(record.withAction(SourceRecord.Action.DELETE), augmented, ingestTime);
    }
