...
public final class TableSchema {
     ...
     * consistent even for a configuration {@link com.fix42.dashboard.connectors.config.ConnectorValidator}
    ...
    public static TableSchema of(ConnectorProperties connector) {
        ...
        if (isSet(target.getSourceKeyColumn())) {
            columns.add(ColumnSpec.sourceKey(target.getSourceKeyColumn().trim()));
        }
        if (isSet(target.getIngestTimestampColumn())) {
            columns.add(ColumnSpec.ingestTimestamp(target.getIngestTimestampColumn().trim()));
        }
        DeephavenTableType type = target.resolveTableType(connector.getSource().stateful());
