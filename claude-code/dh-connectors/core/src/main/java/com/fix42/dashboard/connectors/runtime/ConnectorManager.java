 ...
 *
 * <p>Starting is retried per connector on every tick, so one connector whose broker is
    ...
    public ConnectorManager(
        ...
        for (ConnectorProperties connector : properties.enabledConnectors()) {
            connectors.add(new Connector(
                    ...
                    // Resolved on each start, so a name that names nothing fails this one
                    // connector the way an unreachable broker does.
                    c -> transforms.resolve(c.getTransforms()),
                    clock));
        }
    }
