...
import com.fix42.dashboard.connectors.decode.DelimitedRecordDecoder;
...
import com.fix42.dashboard.connectors.config.FixValueDecode;
...
class SimulatedSourceTest {
    ...
    void replaysTheSowOnStart() {
        ...
        List<SourceRecord> received = new ArrayList<>();
        try (SimulatedSource subscriber = new SimulatedSource(connector)) {
    ...
    void generatesDecodableFixPayloads() {
        ConnectorProperties connector = TestConnectors.fixOrders();
        String payload = new SimulatedSource(connector).encode(3, 7);

        var fields = new DelimitedRecordDecoder(connector.getSource().fieldSeparator())
                .decode(payload);
    ...
    void generatesDecodableJsonPayloads() {
        ConnectorProperties connector = TestConnectors.jsonTrades();
        String payload = new SimulatedSource(connector).encode(2, 4);
    ...
    void nestsDottedJsonTags() {
        ConnectorProperties connector = TestConnectors.jsonTrades();
        String payload = new SimulatedSource(connector).encode(1, 1);
    ...
    private static List<Map<String, String>> replay(ConnectorProperties connector) {
        ...
        SimulatedSource source = new SimulatedSource(connector);
    ...
    void generatesCodesForDecodedFields() {
        ...
        for (Map<String, String> fields : replay(connector)) {
            sides.add(mapper.map(SourceRecord.of("p"), fields, Instant.EPOCH)
    ...
    void cyclesSmallTablesEvenlyForAnEvenKeyCount() {
        ConnectorProperties connector = TestConnectors.jsonTrades();
        connector.getSource().getAmps().setSow(true);
    ...
    void compositeRecordsCarryDecodableParts() {
        ...
        try (SimulatedSource source = new SimulatedSource(connector)) {
            source.start(records::add);
        }
        RecordDecoder decoder = new com.fix42.dashboard.connectors.decode.RecordDecoderFactory(
    ...
    void isConnectedOnlyWhileRunning() {
        ConnectorProperties connector = TestConnectors.jsonTrades();
        SimulatedSource subscriber = new SimulatedSource(connector);
