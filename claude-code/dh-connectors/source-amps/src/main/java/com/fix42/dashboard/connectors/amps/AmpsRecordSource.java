...
import com.fix42.dashboard.connectors.config.AmpsSourceProperties;
...
/**
 * {@link RecordSource} backed by the 60East AMPS java client.
 *
 * <p>Reads its connection and subscription settings from {@code source.amps}; the wire format
 * and the composite part list stay on the connector, because they describe the payload rather
 * than the transport.
 *
...
public class AmpsRecordSource implements RecordSource {
    ...
    public AmpsRecordSource(ConnectorProperties connector) {
        this.connector = connector;
    }

    @Override
    public void start(RecordHandler handler) throws Exception {
        AmpsSourceProperties source = connector.getSource().getAmps();
        ...
        try {
            ...
            SourceRecord.Action action = switch (command) {
                case Message.Command.SOW,
                     ...
                     Message.Command.DeltaPublish -> SourceRecord.Action.UPSERT;
            ...
            if (connector.getFormat() == SourceFormat.COMPOSITE) {
                ...
                handler.onRecord(SourceRecord.composite(parts, message.getSowKey(), action));
                return;
            }
            handler.onRecord(new SourceRecord(message.getData(), message.getSowKey(), action));
