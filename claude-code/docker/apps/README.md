...
| `market-data-demo` | Historical **market data**: per-minute OHLC bars from parquet files (`YYYY/MM/DD/<SYMBOL>.parquet`, local disk or S3) loaded on demand into `md_bars` / `md_daily_summary`, the `md_*` query API and the candlestick / OHLC / line / area / normalized / volume `market_data_dashboard`. Runs from `docker/docker-compose.market-data.yml` (derived image with boto3; optional MinIO with `--profile s3`), not from `docker-compose.yml`. Data is generated by `python -m market_data_demo generate` — see `market-data-demo/README.md`. |
| `basket-oms-demo` | **Basket OMS demo**: a basket ticket, a basket list and an order blotter with a right-click context menu (modify / route / split / cancel) on mock orders, plus a mock venue that fills them — `oms_baskets` / `oms_orders` / `oms_events` / `oms_executions` / `oms_quotes` input tables, the `oms_*_view` projections, the `oms_*` console API and `basket_oms_dashboard`. Runs from `docker/docker-compose.basket-oms.yml` (stock image, no broker), not from `docker-compose.yml` — see `basket-oms-demo/README.md`. |
